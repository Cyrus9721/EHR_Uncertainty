...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate CLMBR / MOTOR patient representations (for all tasks at once)"
    )
    parser.add_argument(
        "--path_to_database",
        required=True,
    ...
    parser.add_argument(
        "--path_to_models_dir",
        type=str,
        help="Path to directory where models are saved",
    )
    parser.add_argument(
        "--model",
        type=str,
        help="Name of foundation model to load. Options: 'motor' or 'clmbr'",
    )
    parser.add_argument(
        "--is_force_refresh",
        action="store_true",
        default=False,
        help="If set, then overwrite all outputs",
    )
...
if __name__ == "__main__":
    ...
    PATH_TO_REPRESENTATIONS = os.path.join(
        PATH_TO_FEATURES_DIR, f"{MODEL}_features.pkl"
    )

    # Check that requested model exists
    assert os.path.exists(
        PATH_TO_MODEL
    ), f"No model for `{MODEL}` exists @ `{PATH_TO_MODEL}`"
    assert os.path.exists(
        PATH_TO_DICTIONARY
    ), f"No model dictionary for `{MODEL}` exists @ `{PATH_TO_DICTIONARY}`"
    ...
    if IS_FORCE_REFRESH or not os.path.exists(PATH_TO_TASK_BATCHES):
        ...
        logger.critical(
            f"Skipping `clmbr_create_batches` because batches already exist @ {PATH_TO_TASK_BATCHES}"
        )
    ...
    if IS_FORCE_REFRESH or not os.path.exists(PATH_TO_REPRESENTATIONS):
        ...
        assert 0 == os.system(command)
        logger.success(
            f"Finish | Create CLMBR representations @ {PATH_TO_REPRESENTATIONS}"
        )
    else:
        logger.critical(
            f"Skipping `clmbr_compute_representations` because representations already exist @ {PATH_TO_REPRESENTATIONS}"
        )
    logger.success("Done!")
